function CFG = drosophila_tu_unfliltered_transfer_settings(CFG)
...
CFG.grid_memreq = 12;
fprintf('12GB for grid nodes.\n');   

CFG.PAR.bmrm.K = 125;             % number of cutting planes used -1 + aggregated plane
CFG.PAR.bmrm.useMonotConstr = 0;  % optimization with monotonicity constraints


warning('SETTING TRANSFER FILE.');
foo = load([CFG.out_dir 'elegans_final.mat']);
wstar = foo.information{end}.w;

% enable mtl
CFG.PAR.mtl.mtl_enable = 1;
CFG.PAR.mtl.mtl_b = 0.75;  % influence= 1 - b 
%CFG.PAR.mtl.mtl_b = 1.00;  % influence: 0%
CFG.PAR.mtl.mtl_wstar = wstar;

warning('Turn off intron span for ige.');
CFG.PAR.ige_intron_span = 0;


% parameter combinations to be used for independent training
% (typically overwritten)
warning('Using ridiculously low number of training examples with 5 levels!');
CFG.train_params = { ...
  [0.5],   [2.5],   [0.25],  [15], 'QP', [1]; ...  
%  [1],   [5],   [0.5],  [15], 'QP', [1]; ...  
};
...
acc_splice_dir = [data_dir 'acc/'];
don_splice_dir = [data_dir 'don/'];
...
% competitor
CFG.cufflinks_pred_file = sprintf('%sD_melanogaster_L3.6.cufflinks.mat', data_dir);
