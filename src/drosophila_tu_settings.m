function CFG = drosophila_tu_settings(CFG)
...
% C: small, smooth, coupling

% -1 training examples = take all you can get
CFG.train_params = { ...
%  [0.1],   [1.5],   [0.5],  [1900], 'QP', [1]; ... 
%  [0.01],  [0.5],   [0.05], [1000], 'QP', [1]; ... 
%  [1],     [5],     [5],    [1900], 'QP', [1]; ... 
%  [0.01],  [1.5],   [0.5],  [1900], 'QP', [1]; ... 
  [0.1],     [1.5],   [0.5],  [-1], 'QP', [1]; ... 
%  [0.1],     [1.5],   [0.5],  [1000], 'QP', [1]; ... 
};

CFG.num_vald_exm = 0;

warning('Setting lower duality gap bound!');
CFG.PAR.bmrm.eps = 0.005;          % relative gap
...
% conversion from labels to states contains
% various heuristics. One is to extend or shrink
% exon boundaries to maximize the agreement with
% the cover_grad feature. The maximimal win
% size is originally 100 around the label position.
CFG.PAR.labels2states_exon_win = 200;

% dont filter label noise examples out of the
% training set
CFG.PAR.train_filter_label_noise = 1;
CFG.PAR.train_filter_amb_label = 1;
CFG.PAR.train_filter_ige_cover = 1;


CFG.max_train_chunk_len = 200000;
% number of discrete expression levels
CFG.PAR.num_levels = 6; % something like 3-10;
...
CFG.PAR.gene_states_low_cover_cutoff = 15;
...
CFG.cufflinks_pred_file = sprintf('%scuffgenes.mat', data_dir);
%CFG.cufflinks_pred_file = sprintf('%sd_unfiltered_cufflinks.mat', data_dir);
